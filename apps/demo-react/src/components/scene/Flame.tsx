...
const Flame: FC = () => {
  ...
  useEffect(() => {
    ...
    particleLayerRef.current.addFlame(option)
  ...
  useEffect(() => {
    ...
    particleLayer.remove("_flame")
    particleLayer.addFlame(option)
  }, [option])

  return (
    <>
      <div ref={containerRef} className="w-full h-full"></div>
      <Space className={["absolute", "w-44", "h-20", "top-3", "right-2"]}>
        <Card className={["w-44", "h-full"]}>
          <div className="w-30">
            <Typography.Text>尺 寸</Typography.Text>
            <Slider
              min={0}
              max={2}
              step={1}
